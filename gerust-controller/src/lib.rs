...
pub trait Controller<C: Context> {
    ...
    fn new(c: &'static C) -> Self where Self: Sized;
...
impl< C: Context, Params, Result, T: Controller<C, Params=Params, Result=Result> + 'static> ControllerFactory<C, Params, Result> for Factory<C, Params, Result, T> {
...
impl<C: Context, Params: 'static, Result: 'static> ControllerSet<C, Result, Params> {
    ...
    pub fn register_controller<T: Controller<C, Params=Params, Result=Result> + 'static>(&mut self) {
